...
class RelaxedTimeRev_LiftConv2d(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, num_filter_banks, activation  # L
    ):
    ...
    def forward(self, x):
        ...
        relaxed_conv_weights = torch.einsum(
            "ng, nog... -> og...", self.relaxed_weights, filter_bank
        )
        ...
        # ==============================
        out = out.view(out.shape[0], self.out_channels, 2, out.shape[-2], out.shape[-1])
        # ==============================
        if self.activation:
...
class RelaxedTimeRev_GroupConv2d(nn.Module):
    ...
    def generate_filter_bank(self):
        filter_bank = torch.stack(
            [self.kernel, torch.flip(self.kernel, dims=[3, 4])], dim=2
        )
        return filter_bank
    ...
    def forward(self, x):
        ...
        relaxed_conv_weights = torch.einsum(
            "ng, nogi... -> ogi...", self.relaxed_weights, filter_bank
        )
        relaxed_conv_weights.reshape(
            self.out_channels * 2,
            self.in_channels,
            2,
            self.kernel_size,
        ...
        x = x.reshape(x.shape[0], self.in_channels * 2, x.shape[-2], x.shape[-1])
...
class TimeRevNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.model = torch.nn.Sequential(
            RelaxedTimeRev_LiftConv2d(
                in_channels, hidden_dim, kernel_size, num_filter_basis, activation=True
            ),
            RelaxedTimeRev_GroupConv2d(
                hidden_dim, hidden_dim, kernel_size, num_filter_basis, activation=True
            ),
            RelaxedTimeRev_GroupConv2d(
                hidden_dim,
                out_channels,
