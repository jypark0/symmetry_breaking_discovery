...
class DatasetFluid2D(torch.utils.data.Dataset):
    def __init__(self, direc, sample_list, x_range=None, y_range=None):
...
def train_epoch_batch(train_loader, model, optimizer, loss_function, device):
    train_mse = []
    relaxed_weights = []
...
def eval_epoch_batch(valid_loader, model, loss_function, device):
    valid_mse = []
    all_preds = []
    all_trues = []
    with torch.no_grad():
...
def train_model_batch(
    model, train_loader, valid_loader, num_epoch, learning_rate, decay_rate, device
):
    ...
    for epoch in range(num_epoch):
        ...
        model.train()
        rmse = train_epoch_batch(train_loader, model, optimizer, loss_fun, device)
        train_rmse.append(rmse)

        model.eval()
        rmse, preds, trues = eval_epoch_batch(valid_loader, model, loss_fun, device)
        ...
        # Early Stopping
        if len(train_rmse) > 30 and np.mean(valid_rmse[-5:]) >= np.mean(
            valid_rmse[-10:-5]
        ):
            break
        if epoch % 10 == 0:
            print(
                "Epoch {} | T: {:0.2f} | Train MAE: {:0.5f} | Valid MAE: {:0.5f} | LR {:0.6f}".format(
                    epoch + 1,
                    epoch_time / 60,
                    train_rmse[-1],
                    valid_rmse[-1],
